...
Etch supports CSS `@custom-media` definitions so you can name your most common media/container queries once and reuse them throughout your project.
...
This follows the `@custom-media` CSS spec being developed by the CSS Working Group. Etch isn’t inventing a new syntax here — it’s bringing `@custom-media` to Etch users early, ahead of broad native browser support.
...
Etch will extract these definitions and make them available to all other global stylesheets.
...
- **Use `@custom-media`**, then add the name and query condition.
- **When choosing a name, prefix with `--`** (required by the syntax).
- **Use “intent-based” names** for clarity and maintainability
