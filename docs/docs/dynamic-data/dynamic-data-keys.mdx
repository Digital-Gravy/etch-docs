...
The `component` namespace provides access to component metadata and prop defaults from within a component's template. It includes the component's `id`, `name`, `key`, and the resolved default value of every prop via `component.defaults.<propKey>`.
